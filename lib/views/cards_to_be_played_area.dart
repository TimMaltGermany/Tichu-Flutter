...
class CardsToBePlayedArea extends Component with HasGameRef<TichuGame> {
  ...
  Rect intersectionRect = const Rect.fromLTWH(0,0,0,0);
  ...
  final Map<String, Card> cardsInPlayArea = {};
  ...
  Tuple2<CardState, Offset> determineCardStateFromPosition(Card card,
    ...
    if (intersectRect.width > 0 && intersectRect.height > 0) {
      ...
      res = Tuple2(currentCardState, const Offset(0, 0));
    }
  ...
  bool isValidSet(Map<String, Card> cardsInPlayArea) {
    ...
    currentSetOfCardsIsValid =  gameRef.tichuRules.isValidAndHigherSet(cards);
  ...
  List<String> getAndClearCards() {
    for (var card in cardsInPlayArea.values) {
      gameRef.remove(card);
    }
    List<String> cardNames = cardsInPlayArea.keys.toList();
    cardsInPlayArea.clear();
    return cardNames;
  }
}